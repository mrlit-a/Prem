   ...
   try {
      ...
      if (body && isPrefix && commands.includes(command) || body && !isPrefix && commands.includes(command) && setting.noprefix || body && !isPrefix && commands.includes(command) && global.evaluate_chars.includes(command)) {
         ...
         for (let name in is_commands) {
            ...
            if (!m.isGroup && !['owner', 'menfess', 'verify'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) return client.sendMessageModify(m.chat, `🚩 Utilisation du bot dans le chat privé uniquement pour les utilisateurs premium, mise à niveau vers le plan premium uniquement 150 GDES,- pour obtenir des limites de 1K pendant 1 mois.\n\nIf you want to buy contact *${prefixes[0]}owner*`, m, {
               largeThumb: true,
            ...
            let teks = `🚩 *[ ${users.attempt} / 5 ]* Vérifiez le numéro par e-mail, 1 e-mail pour vérifier 1 numéro WhatsApp. Veuillez suivre les étapes suivantes :\n\n– *STEP 1*\n Utilisez la commande *${isPrefix ? isPrefix : ''}reg <email>* pour obtenir un code de vérification par e-mail.\n Exemple : *${isPrefix ? isPrefix : ''}reg andysebastien14@gmail.com*\n\n– *STEP 2*\nOuvrez l'e-mail et vérifiez le message entrant ou dans le dossier spam, après avoir reçu le code de vérification, veuillez envoyer le code au botOuvrez l'e-mail et vérifiez le message entrant ou dans le dossier spam, après avoir reçu le code de vérification, veuillez envoyer le code au bot.\n\n*Note* :\nIgnorer ce message *5x* vous serez banni et bloqué, pour débannir et bloquer des frais de 50 GDES`
            if (users && !users.banned && !users.verified && users.attempt >= 5 && setting.verify) return client.reply(m.isGroup ? m.sender : m.chat, Func.texted('bold', `🚩 [ ${users.attempt} / 5 ] : Vous avez ignoré le message de vérification, mais ne vous inquiétez pas, il y a encore d'autres bots, bannis, merci. (^_^)`), m).then(() => {
               users.banned = true
               users.attempt = 0
               users.code = ''
               users.codeExpire = 0
            ...
            if (!['verify', 'exec'].includes(name) && m.isGroup && users && !users.banned && !users.verified && setting.verify) return client.reply(m.chat, `🚩 Votre numéro n'a pas été vérifié, vérifiez en envoyant *${isPrefix ? isPrefix : ''}reg <email>* en chat privé.`, m)
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && chats && !isPrem && !users.banned && !['chatAI', 'menfess_ev'].includes(name) && setting.groupmode) return client.sendMessageModify(m.chat, `🚩 Utilisation du bot dans le chat privé uniquement pour les utilisateurs premium, mise à niveau vers le plan premium uniquement 150 gdes,- pour obtenir des limites de 1 000 € pendant 1 mois.\n\nSi vous souhaitez acheter, contactez *${prefixes[0]}owner*`, m, {
               largeThumb: true,
               thumbnail: await Func.fetchBuffer('https://telegra.ph/file/0b32e0a0bb3b81fef9838.jpg'),
               url: setting.link
            }).then(() => chats.lastchat = new Date() * 1)
            ...
            if (event.download && users && !users.verified && body && Func.socmed(body) && setting.verify) return client.reply(m.chat, `🚩 Votre numéro n'a pas été vérifié, vérifiez en envoyant *${prefixes[0]}reg <email>* en chat privé.`, m)
